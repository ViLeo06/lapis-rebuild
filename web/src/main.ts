...
<div class="battle-bar"><div><b id="phase">非战斗地图</b><p id="vitals">HP 125 / MP 100</p></div><div id="action-wrap" class="action-wrap" hidden><span>行动槽 <b id="action-state">READY</b></span><progress id="action-gauge" max="100" value="100"></progress></div><div class="battle-actions"><button id="battle" class="accent">进入战斗画面</button><button id="return" hidden>退出战斗</button><button id="attack" hidden>普通攻击</button></div></div><div id="skills" class="skill-row" hidden></div>
<div class="disclaimer"><span class="tag inferred">INFERRED</span> 已按 2003 年资料把“非战斗地图”和“行动槽战斗画面”分开；行动槽存在有历史资料支持，但当前充能速度、敌人 AI、伤害与战斗数值仍为 <span class="tag unknown">UNVERIFIED</span>。</div>
...
<div class="panel compact"><span class="tag verified">VERIFIED</span> 帧索引 / bounds / 地图静态资源 / SPR 顺序<br><span class="tag inferred">INFERRED</span> 角色动作、方向命名、战斗画面工程映射<br><span class="tag unknown">UNVERIFIED</span> 行动槽速度 / 训练敌人 / 伤害 / M3任务 / FOCUS 放置</div></aside></main><footer>私人研究预览·不含登录、遥测或原客户端执行 <span>BUILD 0.4 / FIELD + BATTLE LAB</span></footer>`;
...
async function start(){
  ...
  el<HTMLInputElement>('duration').onchange=e=>{
    ...
    if(Number.isFinite(n)&&n>=16&&n<=2000)scene.duration=n;
  ...
  window.addEventListener('lapis-state',e=>{
    ...
    el('mode-subtitle').textContent=s.inBattleView?'战斗画面：行动槽蓄满后执行一次移动或攻击':'非战斗地图：自由移动与资源诊断';
    ...
    el('raw-timing').textContent=String(s.timing);
    ...
    el('action-state').textContent=s.actionReady?'READY':`${Math.floor(s.action)}%`;
    if(s.effect){
      effect.value=String(s.effect.id);
      el('effect-frame').textContent=`${s.effect.cursor+1} / ${s.effect.length} (SPR ${s.effect.frame})`;
      el('effect-timing').textContent=String(s.effect.rawTiming);
