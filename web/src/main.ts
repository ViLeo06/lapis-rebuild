...
<div class="disclaimer"><span class="tag unknown">UNVERIFIED</span> 训练战斗的木桩、伤害、时序和路径策略不代表原版。真实 MagicRes 仅按已验证 SPR 文件顺序做诊断播放。</div>
<div class="journal"><span class="eyebrow">SESSION LOG</span><p id="notice" role="status">准备中</p><div class="save-tools"><button id="save">保存进度</button><button id="load">读取存档</button><button id="export">导出 JSON</button><label class="file-button">导入 JSON<input id="import" type="file" accept="application/json,.json"></label><button id="npc">查看训练说明</button></div></div></section>
...
<div class="panel compact"><span class="tag verified">VERIFIED</span> 帧索引 / bounds / 地图静态资源 / SPR 顺序<br><span class="tag inferred">INFERRED</span> 角色动作与方向命名<br><span class="tag unknown">UNVERIFIED</span> 时序 / 训练玩法 / FOCUS 放置</div></aside></main><footer>私人研究预览·不含登录、遥测或原客户端执行 <span>BUILD 0.2 / M2 + M3-LAB</span></footer>`;
...
async function start(){
 ...
 el('battle').onclick=()=>scene.enterBattle();el('return').onclick=()=>scene.leaveBattle();el('attack').onclick=()=>scene.attack(null);el('save').onclick=()=>void scene.save();el('load').onclick=()=>void scene.loadSaved();
 el('npc').onclick=()=>notice('训练说明（临时 NPC 对话）：点击木桩选择目标，靠近后攻击。战胜两个木桩后返回并存档。地图和 MagicRes 面板用于真实资源诊断。');
