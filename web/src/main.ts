...
<div class="disclaimer"><span class="tag inferred">RECOVERED</span> 行动槽、interaction→battle-entry 边界、AI program 绑定优先级和 ANI common timing 已接入运行时。当前离线 encounter 决策、具体历史敌人 program 与伤害公式明确属于 <span class="tag unknown">RECONSTRUCTION_POLICY</span>，不冒充原版服务器规则。</div>
...
<div class="panel"><h2>02 <span>地图与碰撞</span></h2><label>地图 <span class="tag verified">VERIFIED</span><select id="map"></select></label><div class="checks"><label><input id="grid" type="checkbox">地图网格</label><label><input id="collision" type="checkbox">通行投影</label><label><input id="anchors" type="checkbox">锚点 / Bounds</label></div><dl><dt>角色像素</dt><dd id="anchor">--</dd><dt>鼠标像素</dt><dd id="mouse">--</dd><dt>参考点击格</dt><dd id="reference">--</dd><dt>路径锚点格</dt><dd id="projection">--</dd><dt>IMF 原值</dt><dd id="imf">--</dd><dt>Tile / Resource ID</dt><dd id="resource">--</dd></dl><p class="hint">路径仍用于内部寻路，但不再在正常画面绘制指引线。Bounds 默认隐藏，仅在调试时手动开启。</p></div>
<div class="panel"><h2>03 <span>MagicRes / FOCUS</span></h2><label>特效资源 <span class="tag verified">SPR ORDER</span><select id="effect"></select></label><div class="playback"><button id="effect-play" class="accent">诊断播放</button><button id="effect-step">下一帧</button></div><dl><dt>Effect frame</dt><dd id="effect-frame">--</dd><dt>ANI timing / interval</dt><dd id="effect-timing">--</dd><dt>Placement</dt><dd>UNVERIFIED</dd></dl><p class="hint">特效诊断 cadence 使用已恢复 common timing consumer；FOCUS 的方向、混合、放置和阶段组合仍未恢复。</p></div>
<div class="panel compact"><span class="tag verified">VERIFIED</span> 帧索引 / bounds / ANI common timing / state 3 受击 / battle-entry 显式 zone<br><span class="tag inferred">RECOVERED</span> 20 格 readiness / AI source precedence / client target candidate draw<br><span class="tag unknown">RECONSTRUCTION_POLICY</span> 离线 encounter authority / 训练敌人实例 / 伤害公式 / M3任务 / FOCUS 放置</div></aside></main><footer>私人研究预览·不含登录、遥测或原客户端执行 <span>BUILD 0.7 / S6 RUNTIME INTEGRATION</span></footer>`;
...
async function start(){
  ...
  window.addEventListener('lapis-state',e=>{
    ...
    el('mode-subtitle').textContent=s.inBattleView?`独立战场：battleZone ${s.battleZoneId??'--'} · ${s.battleEntryProvenance}`:'非战斗地图：自由移动与资源诊断';
