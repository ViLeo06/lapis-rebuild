...
<div class="panel compact"><span class="tag verified">VERIFIED</span> 帧索引 / bounds / 地图静态资源 / SPR 顺序<br><span class="tag inferred">INFERRED</span> 角色动作、方向命名、战斗画面工程映射<br><span class="tag unknown">UNVERIFIED</span> 行动槽速度 / 训练敌人 / 伤害 / M3任务 / FOCUS 放置</div></aside></main><footer>私人研究预览·不含登录、遥测或原客户端执行 <span>BUILD 0.5 / TACTICAL REVIEW v3</span></footer>`;
...
async function start(){
  ...
  window.addEventListener('lapis-state',e=>{
    ...
    el('mode-subtitle').textContent=s.inBattleView?'独立战场：对练场（临时场景选用）·亮色格可移动':'非战斗地图：自由移动与资源诊断';
    ...
    el('action-state').textContent=s.battlePaused?'已暂停':s.busy?'行动中':s.actionReady?'可行动':`${Math.floor(s.action)}%`;
    if(s.effect){
