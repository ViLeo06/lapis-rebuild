...
import {validateQuestState} from './quest.ts';
import type {QuestState} from './quest.ts';
export type Save = { version:1; pack:string; character:string; mapId?:number; x:number;y:number; gold:number; inventory?:Inventory; quest?:QuestState; savedAt:string };
export function validateSave(raw:unknown,pack:string,ids:string[],width:number,height:number):Save {
  ...
  if(typeof s.savedAt!=='string'||!Number.isFinite(Date.parse(s.savedAt)))throw new Error('Invalid save timestamp');
  return {...s,inventory:s.inventory===undefined?initialInventory():validateInventory(s.inventory,s.character),quest:validateQuestState(s.quest)};
}
