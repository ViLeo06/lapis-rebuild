...
export function installSourcePanel(pack:LoadedPack){
  ...
  const meta=document.createElement('p');meta.className='hint';meta.id='source-evidence';meta.textContent=`${content.manifest.evidence} · ${content.summary.npc.npc_count} NPC · ${content.summary.quests.length} Quest 文件`;panel.append(meta);
  ...
  const npcs=content.npcScript.npcs;
  for(const npc of npcs)npcSelect.add(option(`${npc.npc_id} / ${npc.name}`,String(npc.npc_id)));
  const questEntries=Object.entries(content.quests).sort(([a],[b])=>Number(a)-Number(b));
  ...
  function currentQuest():QuestContent{return content.quests[questSelect.value]??questEntries[0][1];}
