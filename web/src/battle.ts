...
export function useAttack(s:BattleState, targetId:string, x:number,y:number, skill:Skill|null): {ok:boolean;message:string} {
  ...
  if(!Number.isFinite(x)||!Number.isFinite(y)||!Number.isInteger(cost)||cost<0)return {ok:false,message:'Invalid action parameters'};
  ...
  if(sid===1301)s.shield=P.shieldDurationMs;
    ...
    else { const damage=sid===1101?P.heavyDamage:sid===1201?P.doubleDamage:P.attackDamage;e.hp=Math.max(0,e.hp-damage);if(s.manaBuff>0)s.mp=Math.min(s.maxMp,s.mp+P.manaReturn); }
  }
...
export function updateBattle(s:BattleState,delta:number,x:number,y:number):void {
  ...
  if(s.enemyClock>=P.enemyIntervalMs){s.enemyClock%=P.enemyIntervalMs;for(const e of s.enemies){if(e.hp<=0)continue;const range=e.role==='melee'?P.meleeRadiusPx:P.rangedRadiusPx;if(Math.hypot(e.x-x,e.y-y)<=range){let damage=P.enemyDamage;if(e.blind>0)damage*=P.damageReduction;if(s.shield>0)damage*=P.damageReduction;s.hp=Math.max(0,s.hp-damage);}}}
  finish(s);
}
