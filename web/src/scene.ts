...
export class LabScene extends Phaser.Scene {
  ...
  inventory=initialInventory();quest=initialQuestState();
  ...
  create(){
    ...
    this.guideLabel=this.add.text(0,0,'M3 引导员\nUNVERIFIED',{fontFamily:'sans-serif',fontSize:'11px',color:'#f1d39a',backgroundColor:'#172322cc',align:'center',padding:{x:5,y:3}}).setOrigin(.5,1);
  ...
  private updateGuideLabel(){if(!this.guideLabel)return;const preferred:Cell=this.mapId===0?[26,23]:[34,39];const cell=closestWalkable(this.currentMap().collision,preferred);const p=referenceCellToScreen(cell);this.guideLabel.setPosition(p[0],p[1]-18).setVisible(this.state.phase!=='active');}
  ...
  setMap(id:number){const next=this.pack.maps[String(id)];if(!next){this.notice(`未知地图 ${id}`);return;}if(this.state.phase==='active'){this.notice('训练战斗中不能切换地图');return;}this.mapId=id;this.mapImage?.setTexture(mapTextureKey(id));this.state=initialState();this.resetPosition();this.setAction('00');this.effectSprite?.setVisible(false);this.effectPlaying=false;this.updateGuideLabel();this.fit();this.notice(`已切换地图 ${String(id).padStart(4,'0')} / ${next.manifest.name}`);}
  setCharacter(id:string){if(!this.pack.animations[id])return;this.character=id;this.inventory=changeRole(this.inventory,id);this.setAction('00');this.route=[];this.state=initialState();this.companion?.setVisible(false);this.updateGuideLabel();}
  ...
  talkGuide(){if(this.state.phase==='active'){this.notice('训练战斗中不能推进引导任务');return;}const result=advanceGuide(this.quest,this.mapId);this.quest=result.state;if(result.transitionMapId!==null)this.setMap(result.transitionMapId);this.updateGuideLabel();this.notice(`${result.message} / UNVERIFIED`);}
  enterBattle(){this.resetPosition();this.state=beginBattle(this.anchor.x,this.anchor.y);this.selectedEnemy='dummy-melee';this.setAction('00');this.updateGuideLabel();this.notice('训练战斗：木桩、伤害与时序均为临时实现，非原版规则。');}
  leaveBattle(){if(this.state.phase==='won')this.gold+=this.state.reward;this.state=initialState();this.route=[];this.setAction('00');this.updateGuideLabel();this.notice('已返回安全诊断区');}
  ...
  makeSave():Save {if(this.route.length)throw new Error('Wait for movement to finish');if(this.state.phase==='active')throw new Error('战斗中不能存档');return {version:1,pack:this.pack.digest,character:this.character,mapId:this.mapId,x:this.anchor.x,y:this.anchor.y,gold:this.gold+(this.state.phase==='won'?this.state.reward:0),inventory:{...this.inventory,owned:[...this.inventory.owned]},quest:{...this.quest},savedAt:new Date().toISOString()};}
  async save(){try{await writeSave(this.makeSave());this.notice('存档已写入浏览器 IndexedDB');}catch(e){this.notice(`存档失败，可使用导出 JSON：${String(e)}`);}}
  restore(raw:unknown){const all=Object.values(this.pack.maps).map(m=>m.manifest.render);const maxWidth=Math.max(...all.map(m=>m.width)),maxHeight=Math.max(...all.map(m=>m.height));const s=validateSave(raw,this.pack.digest,Object.keys(this.pack.manifest.characters),maxWidth,maxHeight);const mapId=s.mapId??this.pack.manifest.map.id;if(!this.pack.maps[String(mapId)])throw new Error('Unknown save map');this.setCharacter(s.character);this.setMap(mapId);if(rawCell(this.currentMap().collision,nearestAnchor(s.x,s.y))!==1)throw new Error('存档位置不可通行');this.anchor={x:s.x,y:s.y};this.gold=s.gold;this.inventory=s.inventory!;this.quest=s.quest!;this.updateGuideLabel();this.notice('存档恢复成功');}
  ...
  snapshot(){const a=this.animation(),i=frameIndex(a,this.direction,this.cursor);const ref=referenceScreenToCell(this.hover.x,this.hover.y),projected=nearestAnchor(this.hover.x,this.hover.y);const tileX=Math.floor(this.hover.x/64),tileY=Math.floor(this.hover.y/32);const ins=this.currentMap().inspector;const tile=ins&&tileX>=0&&tileY>=0&&tileX<ins.width&&tileY<ins.height?ins.cells[tileX*ins.height+tileY]:null;const e=this.effectId===null?null:this.effect();
    return {inventory:{...this.inventory,owned:[...this.inventory.owned]},equipment:equipmentBonus(this.inventory,this.character),quest:{...this.quest},ready:!!this.sprite,mapId:this.mapId,mapName:this.currentMap().manifest.name,character:this.character,slot:this.slot,direction:this.direction,cursor:this.cursor,frame:i,length:a.frames_per_direction,bounds:a.frame_bounds[i],timing:a.raw_timing,duration:this.duration,playing:this.playing,anchor:{...this.anchor},hover:{...this.hover},referenceCell:ref,projectedCell:projected,rawReference:rawCell(this.currentMap().collision,ref),rawAnchor:rawCell(this.currentMap().collision,projected),tile:tile?{x:tileX,y:tileY,resource_id:tile.resource_id,path:tile.directory_path}:null,routeLength:this.route.length,cooldown:this.state.cooldown,phase:this.state.phase,hp:Math.ceil(this.state.hp),mp:this.state.mp,gold:this.gold+(this.state.phase==='won'?this.state.reward:0),enemies:this.state.enemies.map(enemy=>({id:enemy.id,hp:Math.ceil(enemy.hp),x:enemy.x,y:enemy.y})),target:this.selectedEnemy,effect:e?{id:e.resource_id,cursor:this.effectCursor,frame:e.sequence[this.effectCursor],length:e.frame_count,rawTiming:e.raw_timing,playing:this.effectPlaying}:null,fps:Math.round(this.game.loop.actualFps)};
  }
  update(time:number,delta:number){if(!this.sprite)return;const dt=Math.min(delta,100);
    ...
    const before=this.state.phase;updateBattle(this.state,dt,this.anchor.x,this.anchor.y,equipmentBonus(this.inventory,this.character).defense);if(before!==this.state.phase){this.route=[];this.updateGuideLabel();this.notice(this.state.phase==='won'?'训练胜利，结算 +10。可返回并存档。':'训练失败，可重新开始。');}
