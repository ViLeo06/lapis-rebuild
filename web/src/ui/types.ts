...
export interface PlayerHudState {
  name:string; className:string; portraitLabel:string; level?:number;
  hp:number; hpMax:number; mp:number; mpMax:number;
  exp?:number; expMax?:number; atk?:number; def?:number; gold:number;
}
...
export interface BattleStatusView { id:string; label:string; durationText?:string; tone?:'buff'|'debuff'|'control'|'poison'|'system'; }
export interface BattleHudState {
  phase:BattlePhase; readiness:number; readinessMax:number; ready:boolean; busy?:boolean; paused?:boolean;
  targetName?:string; targetHp?:number; targetHpMax?:number; statusText?:string; statuses?:readonly BattleStatusView[];
  canAttack:boolean; canRest:boolean; canReturn:boolean; skills:readonly BattleSkillView[];
}
