...
test('basic attack, mana cost and cooldown',async({page})=>{await ready(page);await page.click('#battle');await page.click('#skill-1101');const s=await snap(page);expect(s.phase).toBe('active');expect(s.mp).toBe(75);expect(s.enemies[0].hp).toBe(56);await page.click('#attack');expect((await snap(page)).enemies[0].hp).toBe(56);});
