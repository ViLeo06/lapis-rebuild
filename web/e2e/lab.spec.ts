...
test('MagicRes diagnostic uses sequential SPR frames without direction claims',async({page})=>{await ready(page);expect(await page.locator('#effect option').count()).toBeGreaterThan(0);await page.selectOption('#effect','1');const before=(await snap(page)).effect!;expect(before.id).toBe(1);await page.click('#effect-step');const stepped=(await snap(page)).effect!;expect(stepped.cursor).toBe((before.cursor+1)%stepped.length);expect(stepped.frame).toBe(stepped.cursor);const started=await page.evaluate(()=>{document.querySelector<HTMLButtonElement>('#effect-play')?.click();return window.lapisDiagnostics!.snapshot().effect?.playing??false;});expect(started).toBe(true);await expect(page.locator('.hint').last()).toContainText('FOCUS');});
