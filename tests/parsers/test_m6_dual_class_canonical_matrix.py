...
class CanonicalMatrixTest(unittest.TestCase):
    ...
    def test_stage_sets_and_visual_contract(self):
        sword=[s['id'] for s in self.d['families']['swordsman']['stages']]
        wizard=[s['id'] for s in self.d['families']['wizard']['stages']]
        self.assertEqual(sword,list(range(100,191,10)))
        self.assertEqual(wizard,list(range(109,200,10)))
        self.assertEqual(self.d['visual']['cross_check'],'200/200 files')
        self.assertEqual(self.d['visual']['slots'],['00','01','02','03','05'])
        self.assertEqual(self.d['visual']['semantics']['03'],'VERIFIED-STATIC-ORIGINAL:hit-reaction')

    def test_progression_rows_complete_but_not_server_truth(self):
        total=0
        for family in self.d['families'].values():
            for s in family['stages']:
                total += len(s['progression']['levels'])
    ...
    def test_original_header_tokens_are_not_relabelled(self):
        sword=self.d['families']['swordsman']['common_ability']
