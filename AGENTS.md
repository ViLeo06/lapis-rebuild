...
2. 默认持续推进低风险、可逆工作，不因部分行为未知或非关键格式未完而停下；原客户端动态观察遵守第 5 节的当前会话授权边界。
...
- 默认只做静态读取/解包/反汇编。CI/GitHub Actions 永远不得执行原 Windows 客户端。
- 只有用户在本轮明确要求直接操作原客户端时，才可在 Windows 当前会话中控制用户已经打开的唯一游戏窗口。逐次刷新画面，先观察再做单步操作；不得启动第二个客户端或把本次授权外推到其他会话。
- 用户明确授权的当前战斗测试可包含常规游戏操作及其可见战斗状态变化；不得借此进行付费、交易、删除、账号安全设置、凭据输入、不可逆存档覆盖或其他超出本轮游戏测试范围的操作。遇到登录/安全/权限提示、未知弹窗或动作后状态不明时停止并重新只读观察。
- 不运行原安装器、未知 DLL、Frida/注入组件或来源不明可执行文件；不绕过认证、不攻击或扫描第三方服务器，不挂载敏感目录。
- 记录本机客户端版本证据、观察范围和版本边界。当前客户端的行为只能说明该次会话所用版本，不能自动升级为固定哈希 2.2、2003 国服或服务器公式的事实。
- 没有本轮明确的当前会话操作授权时，不在普通宿主机上对原客户端做动态输入；若用户仍要求动态考据，先使用一次性、可回滚 Windows VM 并创建快照。
...
- 删除/覆盖不可恢复数据
- 权限变更、付费
- 未获本轮明确授权而在普通宿主机动态操作原客户端；未知二进制始终不得运行
